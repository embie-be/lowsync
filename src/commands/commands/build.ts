...
function transpileJavaScript(source: string): {
  compiled: string;
  map: string;
} {
  ...
  const result = babel.transform(source, {
    configFile: false,
    presets: [
      [require('@babel/preset-env'), { ignoreBrowserslistConfig: true }],
    ],
    sourceMaps: true,
    parserOpts: {
...
export default async function (
  { firmwareFile, firmwareConfig }: BuildOptions,
  flashOptions: any
) {
  let config: any;

  if (!flashOptions) flashOptions = {};
  if (!flashOptions.stock) {
    if (firmwareConfig)
      config = JSON.parse(await fs.readFile(firmwareConfig, 'utf8'));
    else
      throw new RunError(
        'No configuration file specified with --firmware-config=..'
      );

    if (!config) config = {};
    if (!config.lowjs) config.lowjs = {};
  } else {
    config = {
      lowjs: {
  ...
  if (config.lowjs.ide_support && !config.lowjs.pro)
    throw new RunError(
      'Only low.js Professional supports the neonious IDE, please fix the configuration'
    );
  if (config.lowjs.ota_update_support && !config.lowjs.pro)
    throw new RunError(
      'Only low.js Professional supports Over-The-Air updating, please fix the configuration'
    );
  ...
  if (
    ...
    config.settings.wifi.ssid &&
    config.settings.wifi.ssid.length > 32
  )
    throw new RunError(
      'The Wifi SSID in the settings must be a maximum of 32 characters long'
    );
  if (
    config.settings &&
    config.settings.wifi &&
    config.settings.wifi.password &&
    config.settings.wifi.password.length < 8
  )
    throw new RunError(
      'The Wifi password in the settings must be at least 8 characters long'
    );
  ...
  firmware_file = await firmware();

  if (config.modules) {
    return (modules_file = null);
  }
  ...
  if (config.lowjs.system_flash_size == 'all')
    ...
    config.lowjs.system_flash_size =
      (config.lowjs.pro ? 0x600000 : 0x300000) *
      (config.lowjs.ota_update_support ? 2 : 1);
  else {
    config.lowjs.system_flash_size = bytes.parse(
      config.lowjs.system_flash_size
    );
    if (!config.lowjs.system_flash_size || config.lowjs.system_flash_size <= 0)
      throw new RunError(
        'lowjs.system_flash_size of firmware config is invalid.'
      );
  }
  ...
  data.writeUInt8(
    (config.lowjs.ota_update_support ? 16 : 0) |
      ...
      (config.settings &&
      config.settings.wifi &&
      (config.settings.wifi.ssid !== undefined ||
        config.settings.wifi.password !== undefined)
        ? 32
        : 0),
    8
  );
  ...
  for (let i = 0; i < numFiles; i++) {
    ...
    let j;
    for (j = 0; j < 256; j++) if (txtBuf.readUInt8(j) == 0) break;
    let txt = txtBuf.toString('utf8', 0, j);

    if (
      config.static_files !== undefined &&
      txt.substr(0, '/fs/user/'.length) == '/fs/user/'
    )
      continue;
    if (
      config.factory_files !== undefined &&
      txt.substr(0, '/fs_factory/user/'.length) == '/fs_factory/user/'
    )
      continue;
    if (
      config.lowjs.ide_support === false &&
      config.lowjs.pro &&
      txt.substr(0, '/client/'.length) == '/client/'
    )
      continue;
    ...
    let fileData = data.slice(
      0x1f0080 + pos,
      0x1f0080 + pos + (sizeCompressed ? sizeCompressed : size)
    );
  ...
  if (!flashOptions.stock) {
    if (modules_file) {
      ...
      files['/fs_factory/modules.json'] = [
        0,
        Buffer.from(modules_json_file) as any,
      ];
    ...
    if (config.factory_files)
      await walkdir(
        config.factory_files,
        '/fs_factory/user',
        '/fs_factory/user/.build'
      );
    ...
    if (config.lowjs.ide_support === false && config.lowjs.pro)
      files['/client/index.html'] = [
        0,
        Buffer.from(
          '<!DOCTYPE html><html><head><title>low.js / lowsync endpoint</title><style type="text/css">body { background-color: white; } * { font-family: Arial; }</style></head><body><h1>low.js / lowsync endpoint</h1><p>This is the HTTP(S) server which handles lowsync requests.</p></body>'
        ),
      ];
    ...
    if (config.settings) {
      ...
      let settings = JSON.parse(
        files['/fs_factory/settings.json'][1].toString()
      );
      walksettings(settings, config.settings);
      files['/fs_factory/settings.json'][1] = Buffer.from(
  ...
  // Add MD5
  let hash = crypto.createHash('md5').update(data.slice(0x1f0080)).digest();
  ...
  let memLowJS = 0,
    ...
    memSettings = 0;
  for (let i in files) {
    if (i.indexOf('/fs_factory/user/') == 0) memFactory += files[i][1].length;
    else if (i.indexOf('/fs/user/') == 0) memStatic += files[i][1].length;
    ...
    else memLowJS += files[i][1].length;
  }
  memLowJS =
    data.length - 0x1f0080 - memStatic - memFactory - memModules - memSettings;

  console.log('****** Used flash space: ******');
  console.log(
    'low.js code     ' + ('          ' + 0x200000).substr(-9) + ' bytes'
  );
  console.log(
    'low.js data     ' + ('          ' + memLowJS).substr(-9) + ' bytes'
  );
  console.log(
    'Static files    ' + ('          ' + memStatic).substr(-9) + ' bytes'
  );
  console.log(
    'Factory files   ' + ('          ' + memFactory).substr(-9) + ' bytes'
  );
  console.log(
    'Modules         ' + ('          ' + memModules).substr(-9) + ' bytes'
  );
  console.log(
    'Settings        ' + ('          ' + memSettings).substr(-9) + ' bytes'
  );
  let memRequired =
    0x200000 + memLowJS + memStatic + memFactory + memModules + memSettings;
  if (flashOptions.stock) {
    if (config.lowjs.pro) {
      console.log(
        'OTA support     ' + ('          ' + 0x200000).substr(-9) + ' bytes'
      );
      memRequired += 0x200000;
    }
  } else if (config.lowjs.ota_update_support) {
    console.log(
      'OTA support     ' + ('          ' + memRequired).substr(-9) + ' bytes'
    );
    memRequired *= 2;
  }
  if (
    config.lowjs.system_flash_size &&
    memRequired > config.lowjs.system_flash_size
  ) {
    console.log('-------------------------------');
    console.log(
      'Total           ' + ('          ' + memRequired).substr(-9) + ' bytes'
    );
    throw new RunError(
      'Total used flash space is higher than the space reserved with lowjs.system_flash_size'
    );
  }
  if (config.lowjs.system_flash_size) {
    console.log(
      'Reserved        ' +
        ('          ' + (config.lowjs.system_flash_size - memRequired)).substr(
          -9
        ) +
        ' bytes'
    );
    ...
    console.log(
      'Total           ' +
        ('          ' + config.lowjs.system_flash_size).substr(-9) +
        ' bytes'
    );
  } else {
    console.log('-------------------------------');
  ...
  if (firmwareFile) {
    ...
    console.log(
      'Written firmware file ' + firmwareFile + ' (' + data.length + ' bytes)'
    );
  } else return data;
}
